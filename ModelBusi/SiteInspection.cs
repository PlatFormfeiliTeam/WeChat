...
//        }
        ...
        public static string Siteapplyall(string ordercode, WGUserEn user)
        {
            ...
            using (DBSession db = new DBSession())
            {
                ...
                curtime = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                sql = "update list_order set inspsiteapplyuserid='{1}',inspsiteapplyusername='{2}',inspsiteapplytime=to_date('{3}','yyyy-MM-dd HH24:mi:ss'),inspstatus=150 where code='{0}' and inspstatus<=150";
                sql = string.Format(sql, ordercode, userid, realname, curtime);
                int i = db.ExecuteSignle(sql);
                if (i > 0)
                {
...
//        }
        ...
        public static string Passall(string ordercode, WGUserEn user)
        {
            ...
            using (DBSession db = new DBSession())
            {
                ...
                curtime = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                sql = "update list_order set inspsitepassuserid='{1}',inspsitepassusername='{2}',inspsitepasstime=to_date('{3}','yyyy-MM-dd HH24:mi:ss'),inspstatus=160 where code='{0}' and inspstatus<=160";
                sql = string.Format(sql, ordercode, userid, realname, curtime);
                int i = db.ExecuteSignle(sql);
                if (i > 0)
                {
